...
class MateriaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('materias.index')->with('message', 'Materia creada correctamente')->with('level', 'notice');
        } catch (\Throwable $e) {
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            // 5. Redireccionar a la lista de materias
            return redirect()->route('materias.index')->with('message', 'Materia actualizada correctamente')->with('level', 'notice');
        } catch (\Throwable $e) {
            // 6. En caso de error, deshacer la transaccion
            DB::rollBack();
    ...
    public function destroy(string $id)
    {
        $materia = Materia::findOrFail($id);
        $materia->delete();
        return redirect()->route('materias.index')->with('message', 'Materia eliminada correctamente')->with('level', 'notice');
    }
}
